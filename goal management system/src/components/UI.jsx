...
// Checkbox-list multi-select — one popover behind two trigger styles:
//   variant="field" — a full-width control that sits among the CoolSelect
//     fields of a filter panel (Business Prospects)
//   variant="icon"  — a small square button for a table column header (Leads)
// The popover is portalled and positioned off the trigger's own rect rather
// than absolutely positioned inside it: a table header lives inside an
// overflow-x-auto wrapper, which clips (and scrolls away) a plain dropdown.
// An empty `selected` means "no restriction", so callers can treat [] as All.
export function MultiSelect({
  ...
  const updateRect = () => {
    ...
    if (!r) return;
    const margin = 12;
    const w = variant === 'field' ? Math.max(r.width, 200) : 220;
    let left = variant === 'field' ? r.left : r.right - w;
  ...
  const summary = selected.length === 0 ? allLabel
    : selected.length === 1 ? (options.find(o => String(o.value) === String(selected[0]))?.label || selected[0])
    ...
    <>
      {variant === 'icon' ? (
        <button
          ...
          className={`relative inline-flex items-center justify-center w-6 h-6 rounded-lg border transition-all cursor-pointer ${
            open || active
        ...
        >
          <Filter size={12} />
          {active && <span className="absolute -top-1 -right-1 w-3.5 h-3.5 rounded-full bg-blue-600 text-white text-[8px] font-bold flex items-center justify-center leading-none">{selected.length}</span>}
        </button>
      ) : (
        <button
          ref={triggerRef}
          type="button"
          onClick={() => { updateRect(); setOpen(o => !o); }}
          className={`${selectCls} flex items-center justify-between gap-2 text-left ${active ? 'font-semibold' : ''} ${className}`}
        >
          <span className={`truncate ${active ? 'text-slate-900 dark:text-white' : 'text-slate-500 dark:text-slate-400'}`}>{summary}</span>
      ...
      {open && rect && createPortal(
        <>
          ...
          <div
            ...
            className="rounded-xl bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800/80 shadow-2xl z-50 p-2 animate-scale-up text-left normal-case tracking-normal"
          >
            <div className="max-h-64 overflow-y-auto space-y-0.5">
              ...
              {options.length === 0 && (
                <p className="px-2.5 py-3 text-xs text-slate-400 dark:text-slate-600 italic">Nothing to filter by yet.</p>
              )}
            </div>
            {active && (
