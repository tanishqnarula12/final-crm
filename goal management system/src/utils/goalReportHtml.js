...
// content is designed to; PAGE_MIN_H pins the footer to the true bottom of
// the physical page. This only reads as "professional" rather than "empty"
// because the content on every page (goal card size/spacing, KPI tiles,
// etc.) is sized to substantially fill a page on its own — the earlier
// version left the footer to float right after a much smaller amount of
// content, which is what actually caused the empty-space complaint. Fix
// the content size, not the footer behavior.
const PAGE_W = 660;
...
function goalDetailCard(g, c) {
  ...
  const stat = (label, value, color) => `
    <div style="background:white;border:1px solid #e2e8f0;border-radius:9px;padding:13px 15px;">
      <div style="font-size:8.5px;font-weight:700;letter-spacing:0.5px;text-transform:uppercase;color:#94a3b8;">${label}</div>
      <div style="font-size:14px;font-weight:700;color:${color || '#0f172a'};margin-top:4px;">${value}</div>
    </div>`;

  return `
  <div style="background:${theme.bg};border:1px solid ${theme.border};border-radius:16px;padding:26px 28px;margin-bottom:32px;break-inside:avoid;page-break-inside:avoid;">
    <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:12px;">
      <div>
        <div style="font-size:18px;font-weight:800;color:${theme.text};">${goalEmoji(g.name)} ${g.name}</div>
        ${needsKidName(g.name) && g.kidName ? `<div style="font-size:9.5px;font-weight:700;letter-spacing:0.5px;text-transform:uppercase;color:${theme.accent};margin-top:3px;">Kid: ${g.kidName}</div>` : ''}
      </div>
      <div style="text-align:right;flex-shrink:0;">
        <div style="font-size:10.5px;font-weight:700;color:#475569;">Target ${monthLabel(g.targetMonth || 1, g.targetYear)}${c.years > 0 ? ` · ${c.years >= 1 ? c.years.toFixed(1) + ' yrs' : c.months + ' mo'}` : ''}</div>
        <div style="font-size:9px;color:#94a3b8;margin-top:3px;">Created ${goalCreatedLabel(g)}</div>
      </div>
    </div>

    <div style="margin-top:20px;">
      <div style="display:flex;align-items:baseline;justify-content:space-between;margin-bottom:7px;">
        <span style="font-size:9px;font-weight:700;letter-spacing:0.5px;text-transform:uppercase;color:#64748b;">Achievement Progress</span>
        <span style="font-size:13px;font-weight:800;color:${achievementHex(c.achievementPct)};">${c.achievementPct.toFixed(1)}%</span>
      </div>
      <div style="height:10px;border-radius:99px;background:#e2e8f0;overflow:hidden;">
        <div style="width:${Math.min(100, c.achievementPct)}%;height:100%;background:${achievementHex(c.achievementPct)};border-radius:99px;"></div>
      </div>
    </div>
    ...
    <div style="margin-top:16px;background:${achievementBg(c.achievementPct)};border-radius:9px;padding:13px 16px;display:flex;align-items:center;justify-content:space-between;">
      <span style="font-size:9.5px;font-weight:700;letter-spacing:0.5px;text-transform:uppercase;color:#475569;">Lump-Sum Equivalent Required Today</span>
      <span style="font-size:15px;font-weight:800;color:${achievementHex(c.achievementPct)};">${fmtINR(c.lumpSumRequired)}</span>
    </div>
  </div>`;
}

function buildGoalDetailPage(chunk, dateStr, pageNum, totalPages) {
  ...
  return `${pageOpen()}
    ...
    <div style="font-size:10.5px;color:#64748b;margin-top:4px;margin-bottom:22px;">Each goal's funding position under the current plan assumptions</div>
    ${cards}
...
export function buildGoalReportHtml(client) {
  ...
  for (let i = 0; i < goalsCalc.length; i += 2) {
    html += buildGoalDetailPage(goalsCalc.slice(i, i + 2), dateStr, pageNum++, totalPages);
  }
